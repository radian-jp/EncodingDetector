...
unsafe
{
    fixed (byte* pUTF16 = &bytesUTF16[0], pUTF8 = &bytesUTF8[0], pSJIS = &bytesSJIS[0])
    {
        ...
        Console.WriteLine($"EncodingDetector.DecodeAuto(Span UTF-16):{EncodingDetector.DecodeAuto(bytesUTF16)}");
        Console.WriteLine($"EncodingDetector.DecodeAuto(Span UTF-8):{EncodingDetector.DecodeAuto(bytesUTF8)}");
        Console.WriteLine($"EncodingDetector.DecodeAuto(Span Shift-JIS):{EncodingDetector.DecodeAuto(bytesSJIS)}");
        Console.WriteLine($"EncodingDetector.DecodeAuto(pointer UTF-16):{EncodingDetector.DecodeAuto(pUTF16, 100)}");
        Console.WriteLine($"EncodingDetector.DecodeAuto(pointer UTF-8):{EncodingDetector.DecodeAuto(pUTF8, 100)}");
        Console.WriteLine($"EncodingDetector.DecodeAuto(pointer Shift-JIS):{EncodingDetector.DecodeAuto(pSJIS, 100)}");
    }
}
